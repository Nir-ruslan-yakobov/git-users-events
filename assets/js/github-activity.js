...
var GitHubActivity = (function () {
    ...
    var methods = {
        ...
        renderIfReady: function (selector, activity) {
            // console.log(selector, activity)
            if (activity) {
                methods.writeOutput(selector, activity);
            }
        }
    };
    ...
    obj.feed = function (options) {
        ...
        methods.getOutputFromRequest(eventsUrl, function (error, output) {
            ...
            methods.renderIfReady(selector, activity);
        });
    };
...
var templates = {
    Stream: '<div class="list-group card-list-group">{{{text}}}',
    Activity: '<div id="{{id}}" class="list-group-item py-5"><div class="media">\
               <div class="media-object avatar avatar-md mr-4""><span class="octicon octicon-{{icon}}"></span></div>\
               <div class="media-body"><div class="float-right text-small text-gray">{{{timeString}}}</div>{{{userLink}}} {{{message}}}</div>\
               <div class="gha-clear"></div>\
             </div></div>',
    SingleLineActivity: '<div id="{{id}}" class="list-group-item py-5"><div class="media">\
               <div class="media-object avatar avatar-md mr-4""><span class="octicon octicon-{{icon}}"></span></div>\
               <div class="media-body"><div class="float-right text-small text-gray">{{{timeString}}}</div>{{{userLink}}} {{{message}}}</div>\
               <div class="gha-clear"></div>\
             </div></div>',
    ...
    GollumEvent: '{{actionType}} the {{{repoLink}}} wiki<small class="d-block">{{{message}}}</small>',
    IssueCommentEvent: 'commented on {{issueType}} {{{issueLink}}}<small class="d-block">{{comment}}</small>',
    IssuesEvent: '{{payload.action}} issue {{{issueLink}}}<small class="d-block">{{payload.issue.title}}</small>',
    MemberEvent: 'added {{{memberLink}}} to {{{repoLink}}}',
    PublicEvent: 'open sourced {{{repoLink}}}',
    PullRequestEvent: '{{payload.action}} pull request {{{pullRequestLink}}}<small class="d-block">{{payload.pull_request.title}}</small>{{{mergeMessage}}}',
    PullRequestReviewCommentEvent: 'commented on pull request {{{pullRequestLink}}}<small class="d-block">{{comment}}</small>',
    PushEvent: 'pushed to {{{branchLink}}}{{{repoLink}}}<br>\
                <ul class="gha-commits">{{#payload.commits}}<li><small>{{{shaLink}}} {{message}}</small></li>{{/payload.commits}}</ul>\
                <small class="gha-message-commits">{{{commitsMessage}}}</small>',
    ReleaseEvent: 'released {{{tagLink}}} at {{{repoLink}}}<small class="d-block"><span class="octicon octicon-cloud-download"></span>  {{{zipLink}}}</small>',
